...
df = pd.read_csv("expires_8-22.csv")

df = df[df['Owner 1 First Name'] != '  '].reset_index()

#~ dates = pd.to_datetime(df['StatusChangeDate'])

#~ df = df[dates > datetime.date(2016,8,1)]

url = "http://people.yellowpages.com/whitepages?first={}&last={}&zip={}&state=ny"

# url.format("First","Last","Zip")
...
#~ for n in zip(addresses, phones):
	#~ print n[0].lstrip().rstrip() + '\t' + n[1].lstrip().rstrip()
#~ print
...
final_df.to_csv(time.ctime().replace(" ","_")+"_results.csv")
print final_df
